...
class DigitalTwins:
    def __init__(self):
        self.llm = HuggingFaceHub(repo_id="mistralai/Mistral-7B-Instruct-v0.2", huggingfacehub_api_token=os.getenv("HUGGINGFACEHUB_API_TOKEN"))
    ...
    def register_llm_twins(self, name, description):
        ...
        if not matched_file:
            ...
            print("已成功複製模板檔案，新檔案的 ID 是:", file_id)
        else:
            print("找到匹配的文件名，將使用現有檔案...")
    ...
    def prompt_llm_twins(self, role, description, prompt, api_table):
        # 定義 SDGs 的問題
        self.templates_profile = self.templates_profile.format(name=role, description=description)
        question = "我想請教您一些問題。" + prompt

        # Get intent from api_table
        self.api_intent = api_table
        result, str_api = self.intent_recognition(role, question)
